...
async function getActorTrainingData(actor) {
    return actor.getFlag("exalted-training-tracker", "data") || {
 ...
 * ------------------------------------------------------------- */
async function buildTrainingContext(actor) {
  const flags = await getActorTrainingData(actor);

  const sortMode = await getSortMode(actor);
  const sorted = sortTrainingItems(flags.items || [], sortMode);
...
function sortTrainingItems(items, mode) {

  const byCreated = (a, b) => (a.createdDate || "").localeCompare(b.createdDate || "");
  ...
  switch (mode) {
    ...
    case "created-status":
      return [...items].sort((a, b) => byStatus(a, b) || byCreated(a, b));

    case "name-status":
      return [...items].sort((a, b) => byStatus(a, b) || byName(a, b));
...
async function getSortMode(actor) {
  return actor.getFlag("exalted-training-tracker", "sortMode") || "created-status";
}
...
function calculateXPTotals(actor, items, customXP) {
  ...
  const totals = {
    standard: { spent: 0, available: 0, total: actor.system.experience.standard.total ?? 0 },
    exalt:    { spent: 0, available: 0, total: actor.system.experience.exalt.total ?? 0 },
...
function activateTrainingListeners(app, html, actor) {
  ...
  /* ---------------------------
   * 2) MANDATE + BONUS XP INPUTS (FLAG STORAGE)
   * --------------------------- */
  ...
  html.find(".event_TrainingComplete").on("click.training", async (evt) => {
    if (!window.confirm("Are you sure you want to mark this training as completed? This action cannot be undone.")) return;
  ...
  html.find(".event_TrainingDelete").on("click.training", async (evt) => {
    if (!window.confirm("Are you sure you want to delete this training project? This action cannot be undone.")) return;
...
async function refreshTrainingTab(app, html, actor) {
  ...
  const windowContent =
    $html.closest("section.window-content").length
      ? $html.closest("section.window-content")
      : $html.find("section.window-content");

  if (!windowContent.length) {
    console.error("Training Tracker | Could not find window-content for refresh.");
    return;
  }

  const trainingTab = windowContent.find("div.tab[data-tab='training']");
  const context = await buildTrainingContext(actor);
...
Hooks.on("renderExaltedThirdActorSheet", async (app, html, data) => {
    ...
    if (!windowContent || windowContent.length === 0) {
        console.error("Training Tracker | Could not find window-content container.");
        return;
    }
